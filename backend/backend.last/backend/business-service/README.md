...
Gate ANPR/PO OCR requires the native **Tesseract OCR** executable as well as
the Python packages in `requirements.txt`. The container image installs it
automatically. For a Windows standalone run, install Tesseract with its
English language data and add its installation directory (for example,
`C:\\Program Files\\Tesseract-OCR`) to `PATH` before starting Uvicorn.
