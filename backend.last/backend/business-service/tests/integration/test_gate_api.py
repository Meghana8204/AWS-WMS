...
@pytest.mark.anyio
async def test_receiving_grn_confirmation_and_lookup(client: AsyncClient):
    """Test /api/receiving/grn confirmation with string PO ID (PO-1001) and subsequent lookup."""
    grn_payload = {
        "poId": "PO-1001",
        "lines": [
            {
                "itemCode": "BEARING-50MM",
                "quantity": 500.0,
            },
            {
                "itemCode": "GASKET-RUBBER",
                "quantity": 100.0,
            },
        ],
    }
    ...
    # 1. Confirm GRN
    confirm_res = await client.post("/api/receiving/grn", json=grn_payload)
    assert confirm_res.status_code == 200, f"GRN confirm failed: {confirm_res.text}"
    data = confirm_res.json()
    assert data["status"] == "CONFIRMED"
    grn_id = data["grnId"]
    assert grn_id.startswith("GRN-")

    # 2. Get GRN detail
    detail_res = await client.get(f"/api/receiving/grn/{grn_id}")
    assert detail_res.status_code == 200
    detail_data = detail_res.json()
    assert detail_data["grnId"] == grn_id
    assert detail_data["status"] == "CONFIRMED"
    assert len(detail_data["lines"]) == 2
    assert detail_data["lines"][0]["itemCode"] == "BEARING-50MM"
    assert float(detail_data["lines"][0]["receivedQuantity"]) == 500.0
