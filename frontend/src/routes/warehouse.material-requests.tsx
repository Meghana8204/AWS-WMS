...
function WarehouseMaterialRequests() {
  ...
  const startCreating = async () => {
    try {
        const { requestNumber, nextMaterialSequence } = await api.getNextMaterialRequestNumber() as any;
    ...
    >
        ...
        <Card className="mb-8 border-primary/20 shadow-soft animate-in slide-in-from-top-4">
          <CardHeader className="border-b border-border/60 bg-muted/10">
             <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-bold uppercase tracking-wider">Create Stock Requirement</CardTitle>
                <Button variant="ghost" size="icon" className="rounded-full" onClick={() => setIsCreating(false)}>
                    <X className="size-4" />
                </Button>
             </div>
          </CardHeader>
          <CardContent className="p-6">
             <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-3">
                   <div className="flex items-center justify-between">
                      <Label className="text-[10px] font-black uppercase text-muted-foreground">Material Items</Label>
                      <Button type="button" variant="outline" size="sm" className="h-7 rounded-lg text-[10px] font-bold" onClick={addItem}>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                   <Button type="button" variant="ghost" className="rounded-xl" onClick={() => setIsCreating(false)}>Cancel</Button>
      ...
      {loading ? (
        ...
        <Card className="flex h-64 flex-col items-center justify-center p-6 text-center border-dashed border-border/50 bg-muted/20">
          ...
          <p className="text-sm text-muted-foreground/70">Create a new request to notify the procurement team.</p>
        </Card>
      ) : (
            ...
            >
              <div className="flex flex-col p-5 md:flex-row md:items-center">
                <div className="mb-4 flex flex-1 items-start gap-4 md:mb-0">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground font-medium">
                      <span className="flex items-center gap-1"><Building2 className="size-3.5" /> {req.department}</span>
                      <span className="flex items-center gap-1"><Clock className="size-3.5" /> Required by {new Date(req.requiredDate).toLocaleDateString()}</span>
                    </div>
                ...
                <div className="text-right hidden md:block">
                   <p className="text-[10px] uppercase font-black text-muted-foreground mb-1">Created At</p>
                   <p className="text-sm font-bold tabular-nums">{new Date(req.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
            </Card>
          ))}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8">
                  ...
                  <div className="space-y-1">
                    ...
                    {isEditing ? (
                        <Input
                            ...
                            onChange={e => setSelectedRequest({...selectedRequest, requiredDate: e.target.value})}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {isEditing && (
                        <Button type="button" variant="outline" size="sm" className="h-7 rounded-lg text-[10px] font-bold" onClick={addEditItem}>
                            <Plus className="size-3 mr-1" /> Add Item
                        </Button>
                    )}
                  </div>
                          ...
                          <tr key={idx} className="border-b border-border/40 last:border-0 hover:bg-muted/20 transition-colors">
                            ...
                            <td className="p-3">
                                {isEditing ? (
                                    <Input
                                        ...
                                        onChange={e => handleEditItemChange(idx, "materialName", e.target.value)}
                            ...
                            <td className="p-3 text-center">
                                {isEditing ? (
                                    ...
                                    <span className="font-bold text-orange-600 tabular-nums">{Math.floor(item.quantity)}</span>
                                )}
                            </td>
                            <td className="p-3">
                                {isEditing ? (
                                    ...
                                    >
                                        ...
                                        <SelectContent className="rounded-xl">
                                            {UOM_OPTIONS.map((uom) => (
                                                <SelectItem key={uom} value={uom} className="text-[10px] rounded-lg">
                                                    {uom}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    ...
                                    <span className="text-[10px] font-black uppercase text-muted-foreground">{item.uom}</span>
                                )}
                            </td>
                            {isEditing && (
                                <td className="p-2">
                ...
                <div className="space-y-2">
                  ...
                  {isEditing ? (
                    <Textarea
                        ...
                        onChange={e => setSelectedRequest({...selectedRequest, remarks: e.target.value})}
