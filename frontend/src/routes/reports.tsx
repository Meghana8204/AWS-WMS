...
function Reports() {
  const reportTypes = [
    { title: "Inventory Valuation", desc: "Current stock value across all categories.", icon: TrendingUp },
    ...
    { title: "Material Rejection Report", desc: "Quality control failure analysis.", icon: AlertTriangle },
    { title: "Warehouse Occupancy", desc: "Bin and rack utilization trends.", icon: BarChart3 },
    { title: "Procurement Spend", desc: "Monthly authorized spend by department.", icon: TrendingUp },
  ];
    ...
    >
      <div className="mb-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
         {reportTypes.map((report, idx) => (
           <Card key={idx} className="group border-border/40 hover:border-primary/30 transition-all hover:shadow-soft overflow-hidden">
      ...
      <SectionCard title="Scheduled Distributions" description="Automated email reports sent to stakeholders" icon={Clock}>
         <div className="space-y-4">
            <ScheduledItem title="Daily Gate Activity" recipients="Security, Warehouse Mgr" frequency="Daily @ 18:00" />
            <ScheduledItem title="Weekly Stock Level" recipients="Procurement, Inventory Team" frequency="Monday @ 08:00" />
            <ScheduledItem title="Monthly Spend Summary" recipients="Finance Head, CFO" frequency="1st of Month" />
         </div>
      </SectionCard>
    </AppShell>
  );
  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-xl border border-border/60 bg-muted/20">
       ...
       <div className="text-right">
          ...
          <span className="text-[9px] font-bold text-success-foreground bg-success-soft/30 px-1.5 py-0.5 rounded border border-success/20 mt-1 inline-block">ACTIVE</span>
       </div>
    </div>
  );
}
