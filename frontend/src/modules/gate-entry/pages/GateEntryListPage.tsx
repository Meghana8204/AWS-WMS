import React from 'react';
import { useQuery } from '@tanstack/react-query';
import { LogIn, Search, Truck, User } from 'lucide-react';
import { gateEntryService } from '../services/gate-entry-service';
import { Table } from '@/components/common/Table';
import { GateEntry } from '../types';

export const GateEntryListPage: React.FC = () => {
  const { data: entries = [], isLoading } = useQuery({
    queryKey: ['gate-entries'],
    queryFn: gateEntryService.getEntries,
  });

  const columns = [
    {
      header: 'Gate Pass',
      accessor: (item: GateEntry) => (
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 border border-slate-100">
            <LogIn className="w-5 h-5" />
          </div>
          <span className="font-extrabold text-slate-900">{item.gate_entry_number}</span>
        </div>
      ),
    },
    {
      header: 'Vehicle',
      accessor: (item: GateEntry) => (
        <div className="flex items-center gap-2 font-bold text-slate-700">
          <Truck className="w-4 h-4 text-slate-400" />
          {item.vehicle_number}
        </div>
      ),
    },
    {
      header: 'Driver',
      accessor: (item: GateEntry) => (
        <div className="flex items-center gap-2">
          <User className="w-4 h-4 text-slate-400" />
          <span>{item.driver_name}</span>
        </div>
      ),
    },
    { header: 'Supplier', accessor: 'supplier_name' as any },
    {
      header: 'Status',
      accessor: (item: GateEntry) => (
        <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-600">
          {item.status}
        </span>
      ),
    },
  ];

  return (
    <div className="space-y-8 animate-premium-fade">
      <div>
        <div className="flex items-center gap-3 text-slate-400 mb-2">
          <LogIn className="w-5 h-5" />
          <span className="text-xs font-black uppercase tracking-[0.2em]">Gate Operations</span>
        </div>
        <h1 className="text-4xl font-extrabold text-slate-900 tracking-tighter">Warehouse Arrivals</h1>
        <p className="text-slate-500 font-medium italic mt-1">Real-time gate entry and vehicle arrival management</p>
      </div>

      <div className="bg-white/50 backdrop-blur-xl border border-slate-200 rounded-[40px] p-2 overflow-hidden premium-shadow">
        <div className="p-6 flex items-center gap-4">
           <div className="flex-1 relative">
             <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
             <input
               type="text"
               placeholder="Search entries by number, vehicle or driver..."
               className="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-3xl text-sm font-medium focus:ring-2 focus:ring-indigo-500/20 transition-all placeholder:text-slate-400"
             />
           </div>
        </div>

        <Table columns={columns} data={entries} isLoading={isLoading} />
      </div>
    </div>
  );
};
